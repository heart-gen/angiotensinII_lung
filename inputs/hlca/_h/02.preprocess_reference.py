"""Build the HLCA objects the downstream analyses read.

Writes two objects from `inputs/hlca/_m/hlca_full.h5ad`:

  hlca_full.dataset.h5ad         all cells, normalized + HVG. Read by
                                 `cell_communication/_h/00.prepare_ccc_input.py`.
  stroma.hlca_full.dataset.h5ad  stromal subset, additionally PCA'd and
                                 Harmony-corrected (75 iterations -- this is the
                                 expensive half). Read by the disease models.

TWO COPIES EXIST, and they must stay identical:

  heart-gen/angiotensinII_lung      inputs/hlca/_h/02.preprocess_reference.py
  heart-gen/lung-pericyte-analysis  disease_association/_h/00.preprocess_reference.py

The disease analyses moved to the second repository on 2026-09-10, but this is
input preparation rather than analysis and each repository has to be able to
build its own inputs. `--targets` exists so neither pays for the other's half:
the mechanism repository needs only `full`, the disease repository needs `both`.
If you change the cell-type fixes, the study filter or the batch variables here,
change them there too -- the two objects are only comparable if they were built
the same way.
"""
import argparse
import pandas as pd
...
def main():
    ...
    ap.add_argument("--targets", choices=("both", "full", "stroma"), default="both",
                    help="which objects to write; see the module docstring. "
                         "`stroma` is the expensive half (75 Harmony iterations).")
    opt = ap.parse_args()

    fadata, adata = subset_data()

    # Process all data
    if opt.targets in ("both", "full"):
